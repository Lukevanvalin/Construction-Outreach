...
export async function initializeDatabase() {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_interaction_notes_prospect_id ON interaction_notes(prospect_id)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_meeting_transcripts_prospect_id ON meeting_transcripts(prospect_id)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_prospects_status ON prospects(status)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_prospects_introduction_date ON prospects(introduction_date)`;
